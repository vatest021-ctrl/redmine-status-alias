...
> [!IMPORTANT]
> Плагин предназначен для Портала ТП на обычном интерфейсе Redmine. Клиент работает с реальными статусами Redmine, но видит их клиентские названия во всех основных точках отображения и отправки уведомлений.
...
| [Внешние каналы](#внешние-каналы) | email, InTouch, redmine_bots и API для других ботов |
...
- [x] Покрывает стандартный UI, историю, email, InTouch и групповые бот-каналы.

## Риски и решения

| Риск | Как решено | Контроль |
| --- | --- | --- |
| Внутренний статус попадет клиенту | Подмена идет в основных точках Redmine UI, истории, email и InTouch | Проверить карточку, список, историю, уведомления |
...
| Групповой канал не связан с пользователем | Есть режим клиентского отображения для каналов без получателя | Настройка `Применять алиасы для каналов без получателя` |
...
3. `Применять алиасы для каналов без получателя`: включить для групповых чатов и внешних ботов, которые отправляют клиентам без конкретного Redmine-пользователя.
...
flowchart TD
  ...
  D -- "канал без получателя" --> F{"Включен режим клиентского канала?"}
  F -- "нет" --> C
  F -- "да" --> E
  E -- "нет" --> C
...
- для каналов без получателя включен клиентский режим.
...
| InTouch: Telegram, Slack, Matrix, MAX, email | покрыто |
| InTouch group messages через `redmine_bots` | покрыто |
...
| Канал | Как применяется alias |
| --- | --- |
| Redmine Mailer | по `User.current`, который Redmine выставляет в получателя письма |
| InTouch личные сообщения | по конкретному пользователю-получателю |
| InTouch групповые сообщения | как клиентский канал без получателя |
| `redmine_bots` как транспорт InTouch | получает уже подготовленный клиентский текст |
| Собственные боты | через общий API плагина или fallback для `issue.status.name` |

### API для других плагинов

Для отправки конкретному пользователю:
...
Для клиентского группового канала без пользователя:
...
Для прямого получения имени статуса:

```ruby
RedmineStatusAlias::Channel.status_name(issue.status, issue: issue, recipient: user)
```
...
- [ ] Проверить InTouch live-уведомление.
- [ ] Проверить InTouch group-уведомление.
