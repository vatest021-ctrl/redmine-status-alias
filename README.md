...
`redmine_status_alias` показывает клиентам выбранные внешние названия статусов обращений, не создавая альтернативные статусы Redmine и не ломая workflow, историю и отчетность.
...
- [x] Позволяет выбрать клиентские роли только из ролей с правом `Видеть клиентские алиасы статусов`.
- [x] Поддерживает служебные роли без alias-подмены.
- [x] Позволяет клиентам менять статус через стандартный workflow Redmine.
- [x] Позволяет задать клиентские переходы для одинаковых alias-статусов.
- [x] Не создает альтернативные `status_id` и не обходит workflow, журнал, отчеты и бизнес-логику Redmine.
...
| Неклиентская роль случайно выбрана для alias | В настройке клиентских ролей доступны только роли с правом `Видеть клиентские алиасы статусов` | `Администрирование -> Роли и права доступа` |
...
| Нарушится workflow | Плагин не создает свой workflow и работает только с разрешенными Redmine переходами | Проверить переходы workflow Redmine |
| Один alias выбран для нескольких внутренних статусов | Плагин предупреждает администратора и позволяет выбрать реальный target-статус в клиентских переходах | Блок `Клиентские переходы` |
| Клиентский переход обойдет workflow | Плагин применяет правило только если выбранный реальный статус есть в разрешенных переходах Redmine | Проверить workflow клиентской роли |
...
> [!WARNING]
> Если несколько внутренних статусов отображаются одним клиентским alias, настройте правило в блоке `Клиентские переходы`. Тогда клиент увидит один пункт, а Redmine получит конкретный внутренний `status_id`.
...
1. В `Администрирование -> Роли и права доступа` выдайте клиентским ролям право `Видеть клиентские алиасы статусов`.
2. `Клиентские роли`: выберите только нужные роли клиентского просмотра. В списке будут доступны только роли с правом из предыдущего пункта.
3. `Служебные роли без алиасов`: эти роли всегда видят внутренние статусы.
4. `Соответствие статусов`: внутренний статус Redmine -> клиентский статус Redmine.
5. `Клиентские переходы`: если один клиентский статус используется для нескольких внутренних статусов, выберите, какой реальный статус ставить при выборе этого alias клиентом.
...
flowchart TD
  ...
  D -- "выбранная роль клиентского просмотра" --> E{"Есть alias для status_id?"}
  E -- "нет" --> C
  E -- "да" --> G["Показать клиентский alias"]
  G --> H{"Клиент меняет статус?"}
  H -- "нет" --> I["Только отображение"]
  H -- "да" --> J{"Alias неоднозначный?"}
  J -- "нет" --> K["Отправить обычный status_id"]
  J -- "да" --> L["Взять target из клиентских переходов"]
  L --> M{"Target разрешен workflow?"}
  M -- "да" --> N["Отправить выбранный внутренний status_id"]
  M -- "нет" --> K
```
...
- текущий пользователь имеет выбранную клиентскую роль в этом проекте;
- выбранная клиентская роль имеет право `Видеть клиентские алиасы статусов`;
- у текущего пользователя нет служебной роли без alias;
- для внутреннего статуса задан alias.

В формах смены статуса плагин дополнительно группирует одинаковые клиентские alias. Если для текущего внутреннего статуса и выбранного клиентского alias задано правило `Клиентские переходы`, в форму попадет выбранный реальный target-статус. Если target не разрешен стандартным workflow Redmine, правило игнорируется и workflow остается источником правды.
...
- [ ] Выдать клиентской роли право `Видеть клиентские алиасы статусов`.
- [ ] Выбрать клиентскую роль в настройках плагина.
...
- [ ] Для одинаковых alias настроить `Клиентские переходы`.
...
Плагин не создает альтернативный workflow. Все переходы, фильтры, отчеты и история продолжают работать по реальным статусам Redmine. Меняется строковое отображение имени статуса для выбранной аудитории, а в формах смены статуса одинаковый клиентский alias может быть привязан к конкретному реальному target-статусу. Такой target применяется только если он уже разрешен стандартным workflow Redmine.
...
В списках одинаковые alias допустимы. Для форм смены статуса используйте блок `Клиентские переходы`: укажите, какой внутренний статус надо поставить, если клиент из конкретного текущего статуса выбирает общий alias.

Пример:

| Если текущий статус | Клиент выбирает | Реально поставить |
| --- | --- | --- |
| `Ожидает клиента` | `В работе` | `Возврат на анализ` |
| `Новая` | `В работе` | `Диагностика у 1 линии` |

Если правило не задано или выбранный target запрещен workflow, Redmine работает по обычному списку разрешенных переходов.
